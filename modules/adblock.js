...
function startAdBlocking() {
    ...
    adSkipInterval = setInterval(() => {
        try {
            ...
            for (const sel of skips) { const btn = document.querySelector(sel); if (btn && btn.offsetParent !== null) { btn.click(); log('Skipped ad'); break; } }
            const vp = document.querySelector('ytd-player');
            if (vp?.shadowRoot) { const sb = vp.shadowRoot.querySelector('.ytp-skip-ad-button'); if (sb && sb.offsetParent !== null) { sb.click(); log('Skipped ad (shadow)'); } }
            if (videoEl && videoEl.duration && videoEl.duration < AD_MAX_DURATION && videoEl.duration > 0) { adVideoDetected = true; cancelRedirect(); videoEl.currentTime = videoEl.duration - 0.1; } else adVideoDetected = false;
        } catch(e) {}
    }, 200);
    const hideAds = () => {
